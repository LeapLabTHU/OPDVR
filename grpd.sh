...
# *** OPDVR: the ReLU gate ***
export CORRECTNESS_GATED=${CORRECTNESS_GATED:-True}     # True = OPDVR; False = vanilla OPD baseline
...
export GRPO_NORM_BY_STD=${GRPO_NORM_BY_STD:-True}   # False = Dr.GRPO style (paper); True = /std
export GRPO_SCALE_BASELINE=${GRPO_SCALE_BASELINE:-0.0}
...
export ACTOR_MODEL_PATH=${ACTOR_MODEL_PATH:-Qwen/Qwen3-4B-Non-Thinking}
export REWARD_MODEL_PATH=${REWARD_MODEL_PATH:-Qwen/Qwen3-4B-Non-Thinking-RL-Math}

export PROJECT_NAME=${PROJECT_NAME:-OPDVR}
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-grpd_${LOG_PROB_TOP_K}_${ACTOR_MODEL_PATH##*/}_$(date +%Y%m%d_%H%M%S)}
